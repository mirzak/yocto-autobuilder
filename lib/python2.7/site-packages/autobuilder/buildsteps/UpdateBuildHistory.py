'''
Created on Jan 12, 2015

__author__ = "Elizabeth 'pidge' Flanagan"
__copyright__ = "Copyright 2012-2015, Intel Corp."
__credits__ = ["Elizabeth Flanagan"]
__license__ = "GPL"
__version__ = "2.0"
__maintainer__ = "Elizabeth Flanagan"
__email__ = "elizabeth.flanagan@intel.com"
'''


from buildbot.steps.shell import ShellCommand
from buildbot.process.buildstep import LogLineObserver
from autobuilder.config import *
import os

class UpdateBuildHistory(ShellCommand):
    haltOnFailure = False
    flunkOnFailure = False
    name = "Update Build History"
    def __init__(self, factory, argdict=None, **kwargs):
        self.machines=""
        self.factory = factory
        for k, v in argdict.iteritems():
            setattr(self, k, v)
        # Timeout needs to be passed to LoggingBuildStep as a kwarg
        self.timeout = 100000
        kwargs['timeout']=self.timeout
        ShellCommand.__init__(self, **kwargs)

    def start(self):
        if os.environ.get('BUILD_HISTORY_WHITELIST') is not None:
            history_repos=ast.literal_eval(os.environ.get('BUILD_HISTORY_WHITELIST').encode('utf-8'))
            if history_repos is not None and repo in history_repos.keys():
                bhw_dir = self.getProperty("BUILD_HISTORY_WEB_DIR")
                bh_dir = self.getProperty("BUILD_HISTORY_DIR")
                pokydir=os.path.join(os.path.join(YOCTO_ABBASE, "yocto-worker"), self.getProperty("buildername")) + "/poky"
                for machine in self.machines
                    self.command =+ os.path.join(bhw_dir + "/warningmgr/import.py") + " -m " + \
                                 machine + " -b " + machine + " " pokydir + \
                                 " " + bh_dir + "/" + machine + ";"
        else:
            self.command = "echo 'Skipping Step.'"
        ShellCommand.start(self)

