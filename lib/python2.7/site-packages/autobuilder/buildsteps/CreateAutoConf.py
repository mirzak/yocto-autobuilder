...
class CreateAutoConf(ShellCommand):
    ...
    def start(self):
        ...
        elif self.atext != "#TOASTER":
            fout = fout+"\n"+self.atext
        else:
            if os.environ.get('TOASTER_UPLOAD_URL', None) is not None:
                fout += 'INHERIT += "toaster"\n'
                # toaster requires build history, so set the markers
                self.buildhistory = True
                os.environ["BUILD_HISTORY_COLLECT"]="True"
...
# us from having a trashed history from "junk" builds (MUTs/release branches/etc)
            if os.environ.get('BUILD_HISTORY_WHITELIST') is not None:
                ...
                if history_repos is not None and repo in history_repos.keys():
                    if self.getProperty("branch") in history_repos[self.getProperty("repository")] and \
                       self.buildhistory == "True" and os.environ.get("BUILD_HISTORY_COLLECT") == "True":
                        fout += 'INHERIT += "buildhistory"\n'
                        fout += 'BUILDHISTORY_COMMIT = "1"\n'
                        fout += 'BUILDHISTORY_DIR = "' + os.environ.get('BUILD_HISTORY_DIR') + '/' + \
                                    self.getProperty("buildername") + '/poky-buildhistory"\n'
                        fout += 'BUILDHISTORY_PUSH_REPO = "' + os.environ.get('BUILD_HISTORY_REPO') + ' ' + \
                                    self.getProperty("buildername") + ':' + self.getProperty("buildername") + '"\n'
