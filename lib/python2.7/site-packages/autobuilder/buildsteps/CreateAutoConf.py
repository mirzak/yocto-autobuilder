...
# us from having a trashed history from "junk" builds (MUTs/release branches/etc)
            ...
            if os.environ.get('BUILD_HISTORY_WHITELIST') is not None or \
               os.environ.get('BUILD_HISTORY_THROWAWAY_WHITELIST') is not None and \
               (self.buildhistory == "True" and os.environ.get("BUILD_HISTORY_COLLECT") == "True"):
                ...
                if self.getProperty("KEEP_BUILD_HIST"):
                    ...
                    self.command += "if [ ! `ls " + bh_repo_dir +"` ];"
                    self.command += "then mkdir -p " + bh_repo_dir +"; fi;"
                    ...
                    self.command += "if [ ! `git ls-remote " + os.environ.get('BUILD_HISTORY_REPO') + \
                                     "--exit-code refs/heads/" + bh_path + "|cut -f2|sed 's/refs\/heads\///g'` ];"
        ...
        self.command += "rm -rf ./build/conf/auto.conf; printf '" + fout + "'>> ./build/conf/auto.conf"
