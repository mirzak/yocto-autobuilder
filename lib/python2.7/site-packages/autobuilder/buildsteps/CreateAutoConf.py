...
class CreateAutoConf(ShellCommand):
    ...
    def start(self):
        ...
        if release_me != 'None':
            self.setProperty('is_release', release_me)

        release = (self.getProperty("is_release") == "True")
        if release:
            sstate_release_number = self.get_release_number()

        if release and int(layerversioncore) > 8 and sstate_release_number == "":
        ...
        if self.atext == "#TOASTER":
            ...
            if self.multilib and self.multilib != "False":
                ...
                if not self.nosstate:
                    ...
                    if release and int(layerversioncore) > 8:
            ...
            if release and int(layerversioncore) > 8 and sstatemirror:
...
# us from having a trashed history from "junk" builds (MUTs/release branches/etc)
        ...
        # Disabled for 2.3 M1 as tests need fixing
        if False and sstatemirror and layerversioncore and int(layerversioncore) > 8:
            sout = 'SSTATE_MIRRORS ?= "\\\n'
