    ...
    def start(self):
        ...
        if int(self.layerversion_core) < 4:
            ...
            if str(os.environ.get('ERROR_REPORT_COLLECT')) == "True":
                if os.environ.get('ERROR_REPORT_SUBMITTER_ID') is not None:
                    self.command = "if [ ! -f ~/.oe-send-error ]; then echo \""+os.environ.get('ERROR_REPORT_SUBMITTER_ID')+"\" > ~/.oe-send-error; fi ; "
	        else:
