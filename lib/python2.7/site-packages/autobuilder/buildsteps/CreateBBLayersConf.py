    ...
    def start(self):
        ...
        if self.bbtext:
            ...
            if self.buildprovider == "yocto":
                if layerversionyocto is not None and int(layerversionyocto) > 5:
            ...
            if self.bsplayer == True:
                if self.bspprovider == "intel":
                    intelbspname = buildername.replace("-server", "").replace("-gladden", "").replace("-noemgd", "").replace("-lsb", "")
                    if intelbspname not in ldirs:
                        fout = fout + workdir + '/build/meta-intel/meta-' + intelbspname + ' \ \n'
                        ldirs.append(intelbspname)
