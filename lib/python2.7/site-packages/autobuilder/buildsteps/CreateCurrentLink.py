...
class CreateCurrentLink(ShellCommand):
    ...
    def start(self):
        ...
        if str(os.environ.get('PUBLISH_BUILDS')) == "True" and str(self.deploycheck) == "False":
            DEST=self.getProperty('DEST')
            self.command = "rm -rf " + DEST + "/../CURRENT; ln -s "+ DEST + " " + DEST+"/../CURRENT"
        else:
            self.command = "echo 'We won't be Publishing this build either because PUBLISH_BUILDS is False or this is a non-publish build'"
