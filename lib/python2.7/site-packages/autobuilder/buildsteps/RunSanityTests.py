...
class RunSanityTests(BitbakeShellCommand):
    ...
    def commandComplete(self, cmd):
        # We want to preserve the original auto.conf, not the modifications made
        # here, for future steps
        localconf = self.getProperty("LOCALCONF")
        workdir = self.getProperty("workdir")
        autoconf = os.path.join(workdir, 'build/build/conf/auto.conf')
        os.remove(autoconf)
        with open(autoconf, 'w') as f:
            f.write(localconf)
        # Chain up to commandComplete in the parent class
        super(RunSanityTests, self).commandComplete(cmd)
