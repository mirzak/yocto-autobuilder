...
class PublishArtifacts(ShellCommand):
        ...
        if str(os.environ.get('PUBLISH_BUILDS')) == "True" and (deploy == "True" or deploy == None):
            for artifact in self.artifacts:
                ...
                if artifact == "adt-installer":
                    ...
                    command=command+"cp -R --no-dereference --preserve=links " + \
                                    os.path.join(self.tmpdir, "deploy/sdk/*buildtools*") + \
                                    " " + DEST + "/buildtools;"
                elif artifact == "rpm":
                    ...
                    command=command+"cp -R --no-dereference --preserve=links " + \
                                    os.path.join(self.tmpdir, "deploy/sdk/*-nativesdk-libc-i686* ") + \
                                    os.path.join(DEST, X86TC_PUBLISH_DIR) + ";"
                    command=command+"cp -R --no-dereference --preserve=links " + \
                                    os.path.join(self.tmpdir, "deploy/sdk/*-nativesdk-libc-x86-64* ") + \
                                    os.path.join(DEST, X8664TC_PUBLISH_DIR) + ";"
                    ...
                    command=command+"mkdir -p " + DEST + "/ptest/machines/" + artifact_name + ";"
                    command=command+"cp -R --no-dereference --preserve=links " + \
                                    deploy_image_dir + \
                                    "/" + artifact_name + "/* " + DEST + "/ptest/machines/" + artifact_name + ";"
                    ...
                    command = command + "echo 'MD5sums are generated and deployed from the image artifact';"
                elif artifact == "None":
    ...
    def getDeployNames(self, artifact, buildername):
        ...
        if artifact_name is "md5sums":
            ...
            artifact_name = artifact_name.replace('ptest-', '')
            deploy_dir_image = os.path.join(self.tmpdir, "deploy-ptest/images/")
        else:
            if self.layerversion_core is not None and int(self.layerversion_core) > 2:
